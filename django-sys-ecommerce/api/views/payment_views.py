...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            # 3. 결제 저장 (serializer 사용 가능)
            payment_serializer = self.get_serializer( # PaymentSerializer
                data = {
                    "imp_uid": imp_uid,
                    "user": user.id, # Order 모델 안에 있는 값에 알아서 찾아간다.
                    "order": order.id, 
                    "paid_amount": paid_amount, # Order 모델 안에 있는 값에 알아서 찾아간다.
                }
            )
